// Content for the info cubes. Edit here; both the client card and the server's cube list read it.
// Colours are not per cube: the client assigns them from CUBE_COLORS by index.
export interface CubeContent {
  id: string;
  h1: string;
  h2?: string;
  h3?: string;
  description: string[];
  logo?: string;
}

export const CUBES: CubeContent[] = [
  {
    id: "aws",
    h1: "Amazon Web Services",
    h2: "Software Dev Engineer Intern",
    h3: "2024",
    description: [
      "My first experience in big tech!",
      "Worked in a (at the time) new org called DataZone, a data governance platform for AWS",
      "Main project was pretty inconsequential, but I got to do some cool stuff with Roaring Bitmaps in Golang at the end",
    ],
    logo: "/assets/logos/aws.png",
  },
  {
    id: "drager",
    h1: "Dräger",
    h2: "Software Engineer Intern",
    h3: "2023",
    description: [
      "My first experience in software!",
      "Built an internal desktop app for defect tracking via Jira in C#",
    ],
    logo: "/assets/logos/drager.png",
  },
  {
    id: "uchicago-cs",
    h1: "UChicago Department of Computer Science",
    h2: "Teaching Assistant / Undergraduate Researcher",
    h3: "2024 - 2026",
    description: [
      "Got interested in databases and did infra work under Prof. Aaron Elmore",
      "TAed a couple times for CMSC 23500 / 33550 undergrad + grad database systems",
    ],
    logo: "/assets/logos/uchicago_cs.png",
  },
  {
    id: "arrowstreet",
    h1: "Arrowstreet Capital",
    h2: "Quant Dev Intern",
    h3: "2025",
    description: [
      "Worked in the research group at a hedgefund solving problems at the intersection of engineering and finance",
      "Walked around everywhere with a notebook and no laptop :D",
      "Participated in daily standups where we weren't allowed to sit down :(",
    ],
    logo: "/assets/logos/arrowstreet.jpg",
  },
  {
    id: "notion",
    h1: "Notion",
    h2: "Software Engineer Intern",
    h3: "2025",
    description: [
      "Took some time off school to intern at Notion in the fall",
      "Built a system called DISS to autonomously detect database integrity problems like schema drift and index invalidation",
      "Created a lot of funny slack emojis",
    ],
    logo: "/assets/logos/notion.jpg",
  },
  {
    id: "iop",
    h1: "UChicago Institute of Politics",
    h2: "Software Developer",
    h3: "2023",
    description: [
      "Did a small part-time gig at UChicago IoP",
      "Created a pipeline for extracting and logging info from political flyers to identify false citations",
    ],
    logo: "/assets/logos/iop.jpg",
  },
];

export const CUBE_IDS = CUBES.map((c) => c.id);
